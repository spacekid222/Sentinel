...
export default function Home() {
  ...
  async function fetchAll() {
    setLoading(true)
    const [g, s, sh, inc, po, ar, av] = await Promise.all([
  ...
  // Check if selected guard is unavailable on selected shift date
  const shiftDateStr = shiftForm.start_time.slice(0, 10)
  const selectedGuardUnavailable = shiftForm.guard_id && shiftDateStr
    ? availability.some(a => a.guard_id === shiftForm.guard_id && a.date === shiftDateStr)
    : false
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar { width: 232px; background: var(--bg2); border-right: 1px solid var(--border); display: flex; flex-direction: column; flex-shrink: 0; position: relative; }
        ...
        .nav-group-label { font-size: 10px; font-weight: 700; color: var(--text3); text-transform: uppercase; letter-spacing: 1.2px; padding: 0 8px; margin-bottom: 6px; }
        ...
        .signout-btn { width: 100%; padding: 8px 12px; background: rgba(255,255,255,0.04); border: 1px solid var(--border); border-radius: 7px; color: var(--text3); font-size: 12px; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.15s; font-weight: 500; }
        ...
        .topbar { padding: 0 32px; height: 60px; background: var(--bg2); border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
        .topbar-left { display: flex; align-items: center; gap: 12px; }
        .topbar-title { font-size: 16px; font-weight: 700; color: var(--text); letter-spacing: -0.3px; }
        .topbar-count { font-size: 11px; color: var(--text3); background: rgba(255,255,255,0.06); border: 1px solid var(--border); padding: 2px 8px; border-radius: 20px; font-family: 'JetBrains Mono', monospace; }
        .btn-add { display: inline-flex; align-items: center; gap: 6px; background: var(--accent); color: #fff; border: none; border-radius: 7px; padding: 7px 14px; font-size: 12.5px; font-weight: 600; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; box-shadow: 0 0 16px var(--accent-glow); transition: all 0.15s ease; }
        ...
        .stat-card { background: var(--bg3); border: 1px solid var(--border); border-radius: 12px; padding: 18px 20px; position: relative; overflow: hidden; transition: border-color 0.2s; }
        ...
        .table-wrap th { padding: 11px 20px; font-size: 10.5px; font-weight: 600; color: var(--text3); text-transform: uppercase; letter-spacing: 0.7px; text-align: left; background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--border); white-space: nowrap; min-width: 100px; }
        .table-wrap td { padding: 12px 20px; font-size: 13px; color: var(--text2); border-bottom: 1px solid var(--border); white-space: nowrap; min-width: 100px; }
        ...
        .btn-sm { padding: 4px 10px; border-radius: 5px; border: none; font-size: 11px; font-weight: 600; cursor: pointer; font-family: 'Plus Jakarta Sans', sans-serif; transition: all 0.15s; }
        ...
        .pill { display: inline-flex; align-items: center; padding: 3px 9px; border-radius: 20px; font-size: 11px; font-weight: 600; letter-spacing: 0.2px; }
        ...
        .dash-row { padding: 12px 18px; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 12px; }
        ...
        .overview-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 18px; border-bottom: 1px solid var(--border); }
        ...
        .po-site-item { padding: 12px 16px; cursor: pointer; border-bottom: 1px solid var(--border); transition: background 0.15s; position: relative; }
        ...
        .po-card { background: var(--bg3); border: 1px solid var(--border); border-radius: 9px; padding: 16px 18px; margin-bottom: 12px; transition: border-color 0.15s; }
        ...
        .billing-filter-bar { background: var(--bg2); border: 1px solid var(--border); border-radius: 10px; padding: 16px 20px; display: flex; align-items: flex-end; gap: 16px; margin-bottom: 20px; flex-wrap: wrap; }
        ...
        .filter-group input, .filter-group select { background: var(--bg3); border: 1px solid var(--border); border-radius: 7px; padding: 7px 12px; color: var(--text); font-size: 13px; font-family: 'Plus Jakarta Sans', sans-serif; outline: none; transition: border-color 0.15s; }
        .filter-group input:focus, .filter-group select:focus { border-color: var(--accent); }
        .filter-group select option { background: var(--bg3); }
        .billing-site-block { background: var(--bg2); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; margin-bottom: 14px; }
        ...
        .billing-site-top { padding: 14px 20px; background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
        ...
        .billing-footer { background: linear-gradient(135deg, #1a1f2e 0%, #141820 100%); border: 1px solid rgba(249,115,22,0.2); border-radius: 12px; padding: 18px 24px; display: flex; align-items: center; justify-content: space-between; margin-top: 20px; }
        ...
        .modal-bg { position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; z-index: 200; animation: fIn 0.15s ease; }
        @keyframes fIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes sUp { from { opacity: 0; transform: translateY(12px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
        .modal { background: var(--bg2); border: 1px solid var(--border2); border-radius: 14px; padding: 28px; width: 480px; box-shadow: 0 24px 80px rgba(0,0,0,0.5); animation: sUp 0.2s ease; }
        ...
        .field input, .field select, .field textarea { width: 100%; background: var(--bg3); border: 1px solid var(--border); border-radius: 8px; padding: 9px 13px; color: var(--text); font-size: 13px; font-family: 'Plus Jakarta Sans', sans-serif; outline: none; transition: border-color 0.15s, box-shadow 0.15s; }
        ...
        .btn-cancel { padding: 8px 16px; border-radius: 7px; border: 1px solid var(--border); background: transparent; color: var(--text2); font-size: 13px; font-weight: 500; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.15s; }
        .btn-cancel:hover { background: rgba(255,255,255,0.05); color: var(--text); }
        .btn-save { padding: 8px 18px; border-radius: 7px; border: none; background: var(--accent); color: #fff; font-size: 13px; font-weight: 600; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; box-shadow: 0 0 16px var(--accent-glow); transition: all 0.15s; }
      ...
      <div className="app">
            ...
            {!loading && activeNav === 'dashboard' && (
              <div>
                <div className="stats-grid">
                  ...
                  <div className={`stat-card${expiringGuards.length > 0 ? ' danger' : ''}`}><div className="stat-label">Expiring Licenses</div><div className="stat-num">{expiringGuards.length}</div><div className="stat-sub">Within 30 days</div></div>
                </div>
            ...
            {!loading && activeNav === 'availability' && (
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {availability.length===0 && <tr><td colSpan={4}><div className="empty"><div className="empty-icon">◷</div><div className="empty-title">No unavailability records</div><div className="empty-sub">Mark guards as unavailable when they can't work certain dates</div></div></td></tr>}
                    {availability.map(a=>(
                      <tr key={a.id}>
                        ...
                        <td>{a.reason||<span style={{color:'var(--text3)'}}>No reason given</span>}</td>
                  ...
                  {!selectedSite ? <div className="empty"><div className="empty-title">Select a site</div></div> : (
                    <>
                      <div className="po-panel-header">
                        <div><div className="po-panel-title">{selectedSite.name}</div><div className="po-panel-sub">{sitePostOrders.length} post orders · {selectedSite.address||'No address'}</div></div>
                      </div>
                ...
                {billingBySite.map(row=>(
                  <div key={row.site.id} className="billing-site-block">
                    <div className="billing-site-top">
                      <div><div className="billing-site-name">{row.site.name}</div><div className="billing-site-meta">{row.shifts.length} shifts · {row.totalHours.toFixed(1)} hrs · {row.site.contact_name||'No contact'}</div></div>
        ...
        {showAddShift && <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&setShowAddShift(false)}><div className="modal">
          ...
          {selectedGuardUnavailable && (
            <div className="avail-warn">
              ⚠️ This guard is marked unavailable on this date. You can still schedule them if needed.
            </div>
          )}
          <div className="two-col">
        ...
        {showAddAvailability && <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&setShowAddAvailability(false)}><div className="modal">
          <div className="modal-title">Mark Unavailable</div><div className="modal-sub">Block a guard from being scheduled on a specific date</div>
          ...
          <div className="field"><label>Reason</label><input placeholder="Vacation, sick leave, personal..." value={availabilityForm.reason} onChange={e=>setAvailabilityForm({...availabilityForm,reason:e.target.value})} /></div>
